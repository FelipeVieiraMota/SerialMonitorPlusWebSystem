...
public class ServletAlterarAluno extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("Dentro da ServletAlterarAluno  =)");
        boolean status;
        HttpSession session = request.getSession();
        int txtIdAluno = Integer.parseInt(request.getParameter("txtIdAluno"));
        String txtCodigoDeMatriculaDoAluno = request.getParameter("txtCodigoDeMatriculaDoAluno");
        ...
        if (status == true) {
            System.out.println("Dentro do if true da ServletAlterarAluno =)");
            session.setAttribute("statusAlteracao", true);
            response.sendRedirect("/PresencaAlunos/cadastros/cadastroDeAlunos.jsp");
            destroy();
        } else {
            System.out.println("Dentro do else (false) da ServletAlterarAluno =(");
    ...
    public void destroy() {
        super.destroy();
        log("Destruindo a servlet ServletAlterarAluno");
