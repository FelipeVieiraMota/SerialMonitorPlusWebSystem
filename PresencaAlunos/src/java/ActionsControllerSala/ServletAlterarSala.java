...
public class ServletAlterarSala extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response)
        ...
        SalaVO vo = new SalaVO(txtIdSala,selectTipoDeSala, txtNumeroDaSala, selectAndarDaSala, txtCodigoDaSala, txtNomeDaSala, observacoes);
        SalaBO bo = new SalaBO();
        status = bo.alterarSalaBo(vo);

        if (status == true) {
            System.out.println("Dentro do if true da ServletAlterarSala =)");
            session.setAttribute("statusAlteracao", true);
            response.sendRedirect("/PresencaAlunos/cadastros/cadastroDeSalasDeAula.jsp");
            destroy();
        } else {
            System.out.println("Dentro do else (false) da ServletAlterarSala =(");
    ...
    public void destroy() {
        super.destroy();
        log("Destruindo a servlet ServletAlterarSala");
